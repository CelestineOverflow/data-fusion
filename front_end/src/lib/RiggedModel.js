...
/**
 * @type {{ rotation: { clone: () => any; set: (arg0: number, arg1: any, arg2: number) => void; }; }}
 */
let headBone;
...
/**
 * @type {{ position: { clone: () => any; copy: (arg0: any) => void; }; }}
 */
...
function add_rigged_model(scene) {
    ...
    loader.load("test_ik_astronaut.glb", function (gltf) {
        model = gltf.scene;
        model.traverse((node) => {
...
let base_bone_original_quat = null;
let thig_l_original_quat = null;
let thig_r_original_quat = null;
let shin_l_original_quat = null;
let shin_r_original_quat = null;

function animateModel(positionL, positionR, headRotation, thighquatL, thighquatR, baseBoneQuat, baseBonePosition, shinLQuat, shinRQuat) {
    ...
    // let temp = (shin_original_rotation.x + euler.x);
    // if (temp > 2 && temp < 4) {
    //     temp = 2;
    // }
    // if (temp > 4) {
    ...
    // euler.setFromQuaternion(worldRotation);
    // temp = (shin_original_rotation.x + euler.x);
    // if (temp > 2 && temp < 4) {
    //     temp = 2;
    // }
    // if (temp > 4) {
    ...
    // shinR.rotation.x = temp;

    // if (shinLQuat) {
    //     const eulerShinL = new THREE.Euler();
    //     eulerShinL.setFromQuaternion(shinLQuat);
    //     shinL.rotation.x = - eulerShinL.x - thighL.rotation.x + Math.PI;
    //     shinL.rotation.y = thighL.rotation.y;
    // }
    // if (shinRQuat) {
    //     const eulerShinR = new THREE.Euler();
    //     eulerShinR.setFromQuaternion(shinRQuat);
    //     shinR.rotation.x = eulerShinR.x - thighR.rotation.x;
    //     console.log("shinR.rotation.x: ", shinR.rotation.x);
    //     shinR.rotation.y = thighR.rotation.y;
    // }
    if (base_bone_original_quat == null) {
        base_bone_original_quat = new THREE.Quaternion();
        base_bone_original_quat.copy(base_bone.quaternion);
    }
    if (thig_l_original_quat == null) {
        thig_l_original_quat = new THREE.Quaternion();
        thig_l_original_quat.copy(thighL.quaternion);
    }
    if (thig_r_original_quat == null) {
        thig_r_original_quat = new THREE.Quaternion();
        thig_r_original_quat.copy(thighR.quaternion);
    }
    if (shin_l_original_quat == null) {
        shin_l_original_quat = new THREE.Quaternion();
        shin_l_original_quat.copy(shinL.quaternion);
    }
    if (shin_r_original_quat == null) {
        shin_r_original_quat = new THREE.Quaternion();
        shin_r_original_quat.copy(shinR.quaternion);
    }
    if (baseBoneQuat) {
        let temp = new THREE.Quaternion();
        temp.copy(base_bone_original_quat);
        temp.multiply(baseBoneQuat);
        base_bone.quaternion.copy(temp);
    }
    if (thighquatL) {
        let temp = new THREE.Quaternion();
        temp.copy(thig_l_original_quat);
        temp.multiply(thighquatL);
        thighL.quaternion.copy(temp);
    }
    if (thighquatR) {
        let temp = new THREE.Quaternion();
        temp.copy(thig_r_original_quat);
        temp.multiply(thighquatR);
        thighR.quaternion.copy(temp);
    }
