<script lang="ts">
	...
	import { thighquatL, thighquatR, baseBoneQuat, shinquatL, shinquatR } from '../stores/websocket';
	...
	// Variables to store quaternion values
	let thighL: Quaternion =  new THREE.Quaternion(0, 0, 0, 1);
    let thighR: Quaternion =  new THREE.Quaternion(0, 0, 0, 1);
    let baseBone: Quaternion =  new THREE.Quaternion(0, 0, 0, 1);
    let shinL: Quaternion =  new THREE.Quaternion(0, 0, 0, 1);
    let shinR: Quaternion =  new THREE.Quaternion(0, 0, 0, 1);
    

	// Subscriptions
	const unsubThighL = thighquatL.subscribe((value) => {
		thighL = value;
	});

	const unsubThighR = thighquatR.subscribe((value) => {
		thighR = value;
	});

	const unsubBaseBone = baseBoneQuat.subscribe((value) => {
		baseBone = value;
	});

	const unsubShinL = shinquatL.subscribe((value) => {
		shinL = value;
	});

	const unsubShinR = shinquatR.subscribe((value) => {
		shinR = value;
	});
	...
	function controllerDisconnected(evt: { target: any }) {
		const index = controllers.findIndex((o: { controller: any }) => o.controller === evt.target);
		if (index !== -1) {
	...
	onMount(() => {
		...
		const render = function () {
			...
			let planePosition = new THREE.Vector3().addVectors(position, cameraDirection);

			reflector.position.set(planePosition.x, planePosition.y, planePosition.z);
			...
			plane2.material.needsUpdate = true; // This line is important to update the texture
