...
class MjpegServer:
    def __init__(self, host: Union[str, int] = "localhost", port: int = 8080, certfile=None, keyfile=None) -> None:
    ...
    async def __root_handler(self, _) -> web.Response:
        ...
        for route in self._cap_routes:
            text += f"<a href='https://{self._host[0]}:{self._port}{route}'>{route}</a>\n<br>\n"
    ...
    def start(self) -> None:
        ...
        if self._certfile and self._keyfile:
            for addr in self._host:
                print(f"\nStreams index: https://{addr}:{self._port!s}")
                print("Available streams:\n")
                for route in self._cap_routes: 
                    print(f"https://{addr}:{self._port!s}{route}")
                print("--------------------------------\n")
        else:
            for addr in self._host:
